    ...
    #>
        ...
        If (-Not $Connection) { Write-error "HCX Auth Token not found, please run Connect-HcxServer " } Else {
            ...
            if($PSVersionTable.PSEdition -eq "Core") {
                $results = Invoke-WebRequest -Uri $url -Body $body -Method POST -Headers $Connection.headers -UseBasicParsing -SkipCertificateCheck
            } else {
                $results = Invoke-WebRequest -Uri $url -Body $body -Method POST -Headers $Connection.headers -UseBasicParsing
            }
